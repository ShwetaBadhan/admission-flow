...
class LeadController extends Controller
{
    ...
    public function show(Lead $lead)
    {
        ...
        // 🔐 COLLEGES FILTER LOGIC (Consultant ke liye locked colleges)
        $user = Auth::user();
        ...
        if ($user->hasRole('consultant')) {
            ...
            if ($consultant) {
                $colleges = $consultant->lockedColleges()
            ...
            // Admin/Superadmin: Sab active colleges
        ...
        // ✅ CONSULTANTS LIST - Counselor dropdown ke liye (ALWAYS fetch karein)
        $consultants = Consultant::where('status', true)->orderBy('name')->get();
        ...
        // ✅ $consultants ko compact mein wapas add karein
        return view('pages.leads.lead-details', compact(
            'lead',
            'colleges',      // 🔐 Filtered colleges (consultant ke liye locked)
            'consultants',   // ✅ Counselor dropdown ke liye (sab active consultants)
            'courses',
            'stages',
            'documentTypes',
            'communicationTypes',
    ...
    public function createAdmissionRequest(Request $request, Lead $lead)
    {
        ...
        $collegeRule = 'required|exists:colleges,id';

        // 🔐 Consultant ke liye: Sirf locked college_id allow karein
        if ($user->hasRole('consultant')) {
            $consultant = Consultant::where('email', $user->email)->first();

            if ($consultant) {
                $lockedCollegeIds = $consultant->lockedColleges()->pluck('colleges.id')->toArray();
                $collegeRule = ['required', Rule::in($lockedCollegeIds)];
            }
        }

        $validated = $request->validate([
            'college_id' => $collegeRule,  // ✅ Dynamic rule
